...
class RegisterController extends AbstractController
{
    ...
    public function index(Request $request, EntityManagerInterface $manager, ServiceMail $mail): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $content_mail = 'Merci de votre inscription sur notre site, pour activer votre compte veuillez cliquer sur le lien suivant:
            <br>
            <a href="http://' . $_SERVER['HTTP_HOST'] . '/inscription/' . $user->getEmail() . '/' . $token . '>http://"' . $_SERVER['HTTP_HOST'] . '/inscription/' . $user->getEmail() . '/' . $token . '</a>
            ';

            // $mail->sendMail($mail, $user->getEmail(),$user->getFullName(),'Inscription sur le site Myboutique');
            mail($user->getEmail(), 'Inscription sur le site MyBoutique', $content_mail);

            $this->addFlash(
                'success',
                'le compte ' . $user->getFirstName() . ' ' . $user->getLastName() . ' a été créé et un mail d\'activation'
            );
