...
class ResetPasswordController extends AbstractController
{
    ...
    public function index(Request $request, UserRepository $repo, EntityManagerInterface $manager): Response
    {
        ...
        if ($request->get('email')) {
            ...
            if ($user) {
                ...
                $content_mail = 'Pour réinitialiser le mdp, veuillez cliquer sur le lien suivant
                <br>
                ...
                //$mail->sendMail($content_mail, $user->getEmail(), $user->getFullName(), 'Reinitialisation mdp');
                mail($user->getEmail(), 'Reinitialisation mdp', $content_mail);
    ...
    public function update(Request $request, ResetPasswordRepository $repo, EntityManagerInterface $manager, $token, UserPasswordHasherInterface $passwordHasher)
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            ...
            $this->addFlash(
                'success',
                'Le mot de passe à bien été réinitialisé'
            );
